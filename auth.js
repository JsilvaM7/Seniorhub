/* ══════════════════════════════════════════════════════════════════════════════
   SeniorHub — Auth + Sidebar de Login  |  v5.0
   Funciona localmente (file://) e em servidor
   ══════════════════════════════════════════════════════════════════════════════ */

const CLUBE_CHECKOUT_URL = 'https://pay.hotmart.com/Y104973165O';

/* ── Tenta iniciar Firebase (falha silenciosamente se offline) ──────────────── */
let fbAuth = null;
let fbProvider = null;
...
try {
    ...
    console.warn('[SeniorHub Auth] Firebase não disponível:', e.message);
}
...
window.SeniorAuth = {
    ...
    logout() {
        ...
        else { _currentUser = null; _atualizarTudo(); }
    }
};

/* ── Atualiza toda a UI quando o estado muda ────────────────────────────────── */
...
function _renderHeaderBtn() {
    ...
    if (!user) {
        ...
        const nome   = (user.displayName || 'Usuário').split(' ')[0];
...
/* ══════════════════════════════════════════════════════════════════════════════
   SIDEBAR — cria o painel se não existir
   ══════════════════════════════════════════════════════════════════════════════ */
function _criarSideBar() {
    ...
    ov.id = 'login-overlay';
    ov.onclick = window.fecharSideBar;
...
function _renderSideBarBody() {
    const body = document.getElementById('lsb-body');
    if (!body) return;
    const user = _currentUser;
        ...
        body.innerHTML = `
            <div class="lsb-section">
                <div class="lsb-icon-wrap">
                    <svg width="44" height="44" viewBox="0 0 24 24" fill="none"
                         stroke="#C5A059" stroke-width="1.4"
                         stroke-linecap="round" stroke-linejoin="round">
                        <circle cx="12" cy="8" r="4"/>
                        <path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/>
                    </svg>
            ...
            <div class="lsb-section">
                ...
                <a href="${CLUBE_CHECKOUT_URL}" target="_blank" rel="noopener noreferrer"
                   class="lsb-clube-btn" onclick="window.fecharSideBar()">
        ...
        body.innerHTML = `
            <div class="lsb-section lsb-section--user">
                <img src="${avatar}" alt="${nome}" class="lsb-avatar"
                     onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(nome)}&background=C5A059&color=fff&size=128'">
                <div>
            ...
            <div class="lsb-section">
                <button class="lsb-logout-btn"
                        onclick="window.SeniorAuth.logout(); window.fecharSideBar();">
...
window.fecharSideBar = function () {
    const sb = document.getElementById('login-sidebar');
...
window.renderModalConteudo = function () {
    ...
    if (!user) {
        body.innerHTML = `
            <div style="text-align:center;padding:8px 0 16px;">
                ...
                <h3 style="font-size:18px;font-weight:800;color:var(--text-dark);margin-bottom:10px;">
                    Entre para participar
                </h3>
                <p style="font-size:14px;color:var(--text-muted);margin-bottom:20px;line-height:1.6;">
                    Faça login com o Google para votar e acessar os benefícios.
                </p>
                <button class="lsb-google-btn" style="width:100%;justify-content:center;box-sizing:border-box;"
                        onclick="toggleModal(); setTimeout(window.abrirSideBar, 300);">
                    Entrar com Google
                </button>
    ...
    body.innerHTML = `
        ...
        <p style="font-size:11px;color:var(--text-muted);margin-top:14px;">
            Seu voto ajuda a construir o portal dos seus sonhos.
        </p>`;
};

/* ══════════════════════════════════════════════════════════════════════════════
   INIT — roda assim que o script carrega (não espera DOMContentLoaded)
   ══════════════════════════════════════════════════════════════════════════════ */
function _init() {
    _criarSideBar();
    _renderHeaderBtn();
...
/* Roda imediatamente se DOM já carregou, senão espera */
